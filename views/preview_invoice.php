<?php
    ...
    foreach ($allProductAdjust as $adjustProduct) {
        ...
        $liveCalculation[$incrementCal++] = intval($adjustProduct->totalProduct - $adjustProduct->adjustProduct);
    ...
    if($diff > 0) {
        $foreignPurchasePaid = number_format($purchasePaid/$purchaseInfo[0]->currencyRate, 2);
        $textColor = '';
        if($purchaseInfo[0]->currencyType == "local") {
            if(number_format($purchasePaid, 2) <= $purchaseInfo[0]->calculateTotalLocal) {
                if($overAdjust < 0) {
                    ...
                    } else if(number_format($purchasePaid, 2) > '0.00' && number_format($purchasePaid, 2) < number_format($purchaseInfo[0]->calculateTotalLocal, 2)) {
                       $statusClass = 'ribbonCustomPartial'; $status = 'Partial'; $adjustColor = '#337AB7'; 
                    } else if($purchaseInfo[0]->cStatus == 2) {
                        $statusClass = 'ribbonCustomSent'; $status = 'Sent'; $adjustColor = '#ED6B75';
                    } else if($purchaseInfo[0]->cStatus == 3) {
                        $statusClass = 'ribbonCustomViewed'; $status = 'Viewed'; $adjustColor = '#ED6B75';
                    } else {
                        $statusClass = 'ribbonCustomDraft'; $status = 'Draft'; $adjustColor = '#ED6B75';
                    }
                }
            } else {
                $statusClass = 'ribbonCustomDue'; $status = '<b style="color: #000 !important">Overpaid</b>'; $adjustColor = '#D91E18';
            ...
            if(number_format($foreignPurchasePaid, 2) <= number_format($purchaseInfo[0]->calculateTotal, 2)) {
                if($overAdjust < 0) {
                    ...
                    if(number_format($foreignPurchasePaid, 2) == number_format($purchaseInfo[0]->calculateTotal, 2)) {
                        $statusClass = 'ribbonCustomPaid'; $status = 'Paid'; $adjustColor = '#36C6D3';
                    } else if(number_format($foreignPurchasePaid, 2) > '0.00' && number_format($foreignPurchasePaid, 2) < number_format($purchaseInfo[0]->calculateTotal, 2)) {
                       $statusClass = 'ribbonCustomPartial'; $status = 'Partial'; $adjustColor = '#337AB7'; 
                    } else if($purchaseInfo[0]->cStatus == 2) {
                        $statusClass = 'ribbonCustomSent'; $status = 'Sent'; $adjustColor = '#ED6B75';
                    } else if($purchaseInfo[0]->cStatus == 3) {
                    ...
                    <div class="row">
                        <div class="col-md-2 pull-right" style="z-index: 10000;">
                            <div class="BillView" style="padding: 5px;">                                
                                <div class="btn-group" role="group" aria-label="...">
                                    <a href="<?php echo base_url(); ?>invoice/pdfInvoice/<?php echo $purchaseInfo[0]->id ?>/view" target="_blank" class="btn yellow-mint btn-sm"><i class="fa fa-file-pdf-o"></i> Pdf</a>
                                    <a href="<?php echo base_url(); ?>invoice/printInvoice/<?php echo $purchaseInfo[0]->id ?>" target="_blank" class="btn btn-primary btn-sm"><i class="fa fa-print"></i> Print</a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-9">
                                                                ...
                                                                <?php 
                                                                    ...
                                                                    if($purchaseInfo[0]->currencyType == 'local') {
                                                                        ...
                                                                        echo    '<a class="tooltipC">
                                                                                    ...
                                                                                    <span class="sTooltip"><strong class="currencyText">'.$localSymbol." ".number_format(number_format($purchaseInfo[0]->calculateTotalLocal,2)-number_format($purchasePaid, 2), 2).'</strong></span>
                                                                                </a>';
                                                                    }
                                                                ?>
                                                            </span>
                                                                ...
                                                                <tr>
                                                                    <td>
                                                                        ...
                                                                        <?php if($columnSettings[0]->isWarranty == 1) { ?>
                                                                            <br/><span class="warrantySpan" style="color: gray;"><?php echo $columnSettings[0]->warrantyName; ?>: 
                                                                                <span class="checkWarranty">
                                                                                    <?php
                                                                                        $date1 = date('Y-m-d');
                                                                                        $date2 = date('Y-m-d', strtotime(date("Y-m-d", strtotime($purchaseInfo[0]->createdDateTime))." +".$specificTr[7]));
                                                                                        $period = explode(" ", $specificTr[7]);
                                                                                        if($date1 < $date2) {
                                                                                            ...
                                                                                            if($period[1] == 'Year') {
                                                                                                printf("%d years, %d months, %d days left\n", $years, $months, $days);
                                                                                            } else if($period[1] == 'Month') {
                                                                                                printf("%d months, %d days left\n", $months, $days);
                                                                                            } else {
                                                                                                printf("%d days left\n", $days);
                                                                                            }
                                                                                        } else {
                                                            ...
                                                            <td class="text-right">
                                                                <?php 
                                                                    if($purchaseInfo[0]->discountAmount != 0.00) { 
                                                                        ...
                                                                        if($purchaseInfo[0]->currencyType == 'local') {
                                                                            $currencySysmbol = explode("-", $basicSettings[0]->currencySymbol);
                                                                            echo $currencySysmbol[1]." ".$discount, 2;
                                                                        } else {
                                                                            $currencySysmbol = explode("-", $basicSettings[0]->foreignCurrencySymbol);
                                                                            echo $currencySysmbol[1];
                                                                            echo    '<a class="tooltipC">
                                                            ...
                                                            <td id="paidText" class="text-right"> 
                                                                <?php 
                                                                    ...
                                                                    if($purchaseInfo[0]->currencyType == 'local') {
                                                                        ...
                                                                        echo $currencySysmbol[1];
                                                                        echo    '<a class="tooltipC">
                                                                                    <span><span class="paid">'.number_format(($purchasePaid), 2).'</span></span>
                                                                                    <span class="sTooltip"><strong>'.$localSymbol." ".number_format($purchasePaid*$purchaseInfo[0]->currencyRate, 2).'</strong></span>
                                                                                </a>';
                                                                    }
                                                                ?>
                                                            </td>
                        ...
                        <div class="col-md-3">
                            <div class="BillView">
                                ...
                                <?php $payments = $this->data_model->get_all_info_where_with_order('*', array('documentId' => $this->uri->segment(3), 'docType' => 'invoice', 'type !=' => '', 'status' => 1), 'id', 'DESC', 'list_payment'); ?>
                                </div>
                        ...
                        <div class="col-md-offset-9 col-md-2 pull-right bottomButton" style="display: none">
                            <div class="BillView" style="padding: 5px;">                                
                                <div class="btn-group" role="group" aria-label="...">
                                    ...
                                    <a onclick="data_history('print', 'requisition', 'raw_material', '3')" href="<?php echo base_url(); ?>invoice/printBill" target="_blank" class="btn btn-primary btn-sm"><i class="fa fa-print"></i> Print</a>
                                </div>
                            </div>
                        </div>
                    </div>
