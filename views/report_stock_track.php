...
<html lang="en">
    ...
    <head>
        ...
        <style type="text/css">
            ...
            a.tooltipC { outline:none; font-size: 15px; }
            a.tooltipC strong { line-height:17px; }
            a.tooltipC:hover { text-decoration:none; }
            a.tooltipC .sTooltip { z-index:9999; display:none; padding:6px 6px; margin-top:-20px; margin-left:5px; max-width:225px; height:auto; line-height:0px; }
									...
									<div class="portlet box light bordered">
										...
										<div id="portlet-body" class="portlet-body">
                                            <div style="text-align: center; margin: 10px 0px 25px 0px;">
                                                ...
                                                <span style="font-weight: 600; font-size: 15px;"><b>Stock Track</b><br/></span>
                                                <div class="col-md-12" style="margin: 15px 0px;">
                                                    <div class="col-md-6">
                                                        <?php
                                                            if($productStock[0]->picture != '') {
                                                        ?>
                                                                <img class="img-responsive" style="max-width: 200px; margin-left: auto; margin-right: auto" src="<?php echo base_url(); ?>assets/uploadedFile/productPicture/<?php echo $productStock[0]->picture; ?>" />
                                                        <?php
                                                            } else {
                                                        ?>
                                                                <img class="img-responsive" style="max-width: 200px; margin-left: auto; margin-right: auto" src="<?php echo base_url(); ?>assets/uploadedFile/productPicture/productDefault.jpg" />
                                                        <?php
                                                            }
                                                        ?>
                                                    </div >
                                                    <div class="col-md-6" style="text-align: left; font-weight: normal; font-size: 18px; color: gray">
                                                        <div style="margin: auto; padding-top: 8%">
                                                        Code: <?php if($productStock[0]->code == '') { echo "<b>---</b>"; } else { echo $productStock[0]->code; } ?><br/>
                                                        Name: <?php echo $productStock[0]->name; ?><br/>
                                                        Stock Balance: <?php echo $productStock[0]->balanceQuantity-$productStock[0]->sellingQuantity." ".$productStock[0]->unit; ?> <b>|</b>
                                                        Status: 
                                                        <?php  
                                                            if(($productStock[0]->balanceQuantity-$productStock[0]->sellingQuantity) == 0) {
                                                        ?>
                                                                <a class="tooltipC">
                                                                    <span class="label label-sm" style="background: blue"> N/A </span>
                                                                    <span class="sTooltip"><strong>Total Quantity: <?php echo $productStock[0]->balanceQuantity-$productStock[0]->sellingQuantity." ".$productStock[0]->unit; ?><br/> Alert Quantity: <?php echo $productStock[0]->alertQuantity." ".$productStock[0]->unit; ?></strong></span>
                                                                </a>
                                                        <?php 
                                                            } else if(($productStock[0]->balanceQuantity-$productStock[0]->sellingQuantity) > $productStock[0]->alertQuantity) {
                                                        ?>
                                                                <a class="tooltipC">
                                                        ...
                                                        ?>
                                                        <br/>
                                                        Category: <?php $category = $this->data_model->get_specific_column("name", array("id" => $productStock[0]->category), "settings_product_category"); echo $category[0]->name; ?> <b>|</b>
                                                        Sub-category: 
                                                        <?php
                                                            if($productStock[0]->category != 1) {
                                                                if($productStock[0]->subCategory != '---') {
                                                                    $subCategory = $this->data_model->get_specific_column("name", array("id" => $productStock[0]->subCategory), "settings_product_category"); echo $subCategory[0]->name;
                                                                } else {
                                                                    echo "<b>---</b>";
                                                                }
                                                            } else {
                                                                ...
                                                                <?php
                                                                    if($detail->documentId != 0) {
                                                                        if($detail->type == 'purchase') {
                                                                            $docName = $this->data_model->get_specific_column('documentId', array('id' => $detail->documentId), 'list_document');
                                                                            echo '<a href="'.base_url().'product/viewBill/'.$detail->documentId.'" target="_blank">'.$docName[0]->documentId.'</a>';
                                                                        } else {
                                                                            $docName = $this->data_model->get_specific_column('documentId', array('id' => $detail->documentId), 'list_document');
                                                                            echo '<a href="'.base_url().'invoice/viewInvoice/'.$detail->documentId.'" target="_blank">'.$docName[0]->documentId.'</a>';

                                                                        }
                                                                    } else {
                                                                        echo "<b>Initial Stock</b>";
                                                                    }
                                                                ?>
                                                            </td>
                                                            <td><?php echo ucfirst($detail->type); ?></td>
                                                            <td><?php echo $detail->totalProduct." ".$productStock[0]->unit; ?></td>
                                                            <td>
                                                                <?php 
                                                                    if($detail->type == 'purchase') {
                                                                        echo number_format($detail->totalProduct*$detail->purchasePrice, 2); 
                                                                    } else {
                                                                        echo number_format($detail->totalProduct*$detail->SellingPrice, 2); 
                                                                    }
                                                                ?>
                                                            </td>
                                                            <td>
                                                                <?php $team = $this->data_model->get_specific_column("name", array("id" => $detail->createdBy), "list_team"); echo $team[0]->name; ?>
                                                            </td>
                                                            <td>
                                                                <?php echo date("d-m-Y", strtotime($detail->createdDateTime)); ?>
                                                            </td>
                                                        </tr>
                                                        <?php } ?>
                                                    </tbody>
