                                    ...
                                    <div class="portlet box light bordered">
                                        ...
                                        <div id="portlet-body" class="portlet-body">
                                            <div style="text-align: center; margin: 10px 0px 25px 0px;">
                                                ...
                                                <span style="color: #000">Reporting period: 
                                                    <?php 
                                                        if($dataId[0]->userName != 0) {
                                                            echo $dataId[0]->startDate." to ".$dataId[0]->endDate;
                                                        } else {
                                                            echo $dataId[0]->startDate;
                                                        }
                                                    ?>
                                                </span>
                                            </div>
                                                ...
                                                <table id="table-pagination" class="table table-striped table-hover green">
                                                    <thead>
                                                        <tr style="background-color: #0B5394; color: #FFFFFF">
                                                            <th class="col-xs-1"> Dalivery Date </th>
                                                            <th class="col-xs-1"> Customer Name </th>
                                                            <th class="col-xs-1"> Invoice No. </th>
                                                            <th class="col-xs-2 centerTh"> Sales Value 
                                                                <table>
                                                                    <?php
                                                                        $i = 0;
                                                                        $productCategories = $this->data_model->get_all_info_where_with_order('*', array('parent' => 0), 'sortOrder', '', 'settings_product_category'); 
                                                                        foreach ($productCategories as $categoriesProduct) {
                                                                            if($categoriesProduct->id != 1) {
                                                                    ?>
                                                                            <th class="col-xs-1"><?php echo $categoriesProduct->name; ?></th>
                                                                    <?php
                                                                            }
                                                                        }
                                                                    ?>
                                                            ...
                                                            <th class="col-xs-1 rightTh"> Total Price </th>
                                                            <th class="col-xs-2 rightTh"> Remarks </th>
                                                            <th class="col-xs-2 rightTh"> Check Process </th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <?php $i = 1; $inTotalPrice = 0; $purchasePaid = 0; $totalCollection = 0; foreach ($details as $detail) { 
                                                            $lp = 0; $catInfo = array();
                                                            foreach ($productCategories as $categoriesProduct) {
                                                                if($categoriesProduct->id != 1) {
                                                                    $catInfo[++$lp-1] = $categoriesProduct->name; $catInfo[++$lp-1] = 0;
                                                                }
                                                            }
                                                            if($dataId[0]->type != 'Purchase Statement') {
                                                                $purchaseInfo = $this->data_model->get_all_info('*', array('id' => $detail->id), 'list_document');
                                                                $amountExpense = $this->data_model->columnSum('adjustAmount', array('docType' => 'invoice', 'documentId' => $detail->id, 'type' => 'Expense'), 'list_payment'); 
                                                                $amountIncome = $this->data_model->columnSum('adjustAmount', array('docType' => 'invoice', 'documentId' => $detail->id, 'type' => 'Income'), 'list_payment'); 
                                                                $purchasePaid = $amountIncome[0]->adjustAmount-$amountExpense[0]->adjustAmount;
                                                            } else {
                                                                $purchaseInfo = $this->data_model->get_all_info('*', array('id' => $detail->id), 'list_document');
                                                                $amountExpense = $this->data_model->columnSum('adjustAmount', array('docType' => 'purchase', 'documentId' => $detail->id, 'type' => 'Expense'), 'list_payment'); 
                                                                $amountIncome = $this->data_model->columnSum('adjustAmount', array('docType' => 'purchase', 'documentId' => $detail->id, 'type' => 'Income'), 'list_payment'); 
                                                                $purchasePaid += $purchaseInfo[0]->calculateTotalLocal-($amountExpense[0]->adjustAmount-$amountIncome[0]->adjustAmount);
                                                                // $purchasePaid += $purchaseInfo[0]->calculateTotalLocal - $purchasePaid;
                                                                // echo $detail->id."  ".($purchasePaid);
                                                            }
                                                        ?>
                                                        <tr>
                                                            <td><b><?php echo date("d-m-Y", strtotime($detail->createdDateTime)); ?> </b></td>
                                                            <td>
                                                                <?php 
                                                                    $name = $this->data_model->get_specific_column('name', array('id' => $detail->customerId), 'list_business_member'); 
                                                                    echo $name[0]->name;
                                                                ?>
                                                            </td>
                                                            <td> 
                                                                <?php 
                                                                    if($dataId[0]->type != 'Purchase Statement') {
                                                                ?>
                                                                    <a href="<?php echo base_url();?>invoice/viewInvoice/<?php echo $detail->id; ?>" target="_blank"><?php echo $detail->documentId ?> </a>
                                                                <?php
                                                                    } else {
                                                                ?>
                                                                    <a href="<?php echo base_url();?>product/viewBill/<?php echo $detail->id; ?>" target="_blank"><?php echo $detail->documentId ?> </a>
                                                                <?php
                                                                    }
                                                                ?> 
                                                            </td>
                                                            <td>
                                                                <?php
                                                                    // echo $detail->documentData;
                                                                    $totalPrice = 0; $subTotal = 0;
                                                                    $dataDoc = explode("::", $detail->documentData); $j = 0;
                                                                    // print_r($detail->documentData) ;
                                                                    foreach ($dataDoc as $datall) {
                                                                        $allData = explode('*', $datall);
                                                                        $catId = $this->data_model->get_specific_column('category', array('id' => $allData[1]), 'list_product_enrollment'); 
                                                                        $catName = $this->data_model->get_specific_column('name', array('id' => $catId[0]->category), 'settings_product_category'); 
                                                                        for ($k = 0; $k < count($catInfo); $k+=2) { 
                                                                            if($catName[0]->name === $catInfo[$k]) {
                                                                                $catInfo[$k+1] += $allData[6];
                                                                            } else {

                                                                            }
                                                                        }
                                                                        // print_r($catName[0]->name);
                                                                    }
                                                                ?>
                                                                <table class="insideTable">
                                                                    <?php 
                                                                        // print_r($catInfo);
                                                                        for ($k = 0; $k < count($catInfo); $k+=2) { 
                                                                            $totalPrice += $catInfo[$k+1];
                                                                            echo '<th class="col-xs-1">'.number_format($catInfo[$k+1], 2).'</th>';
                                                                        }
                                                            ...
                                                            <th class="rightTh">
                                                                Total: <?php echo number_format($totalPrice, 2); ?> <br/>
                                                                Discount: <?php echo number_format(($totalPrice*$detail->discountAmount)/100, 2); ?> <br/>
                                                                <hr class="lineHr" />
                                                                SubTotal: 
                                                                <?php 
                                                                    echo number_format($totalPrice-($totalPrice*$detail->discountAmount)/100, 2); 
                                                                    $inTotalPrice += $subTotal += $totalPrice-($totalPrice*$detail->discountAmount)/100;
                                                                ?>
                                                            </th>
                                                            <td class="rightTh"><?php echo $detail->remarks; ?></td>
                                                            <td>
                                                                <?php
                                                                    if($dataId[0]->type != 'Purchase Statement') {
                                                                        $data = $this->data_model->get_all_info('*', array('docType' => 'purchase', 'docId' => $detail->id, 'commentIcon' => 'fa-exchange'), 'list_all_comment');
                                                                        foreach ($data as $key) {
                                                                            echo $key->comments."<br/>";
                                                                        }
                                                                    } else {
                                                                        $data = $this->data_model->get_all_info('*', array('docType' => 'purchase', 'docId' => $detail->id, 'commentIcon' => 'fa-exchange'), 'list_all_comment');
                                                                        foreach ($data as $key) {
                                                    ...
                                                    <tfoot>
                                                        ...
                                                        <?php 
                                                            if($dataId[0]->type != 'Purchase Statement') {
                                                        ?>
                                                            <th colspan="2">Total Sales <span class="spanAmount"><?php echo number_format($inTotalPrice, 2); ?></span></th>
                                                            <th colspan="2">Total Collection <span class="spanAmount"><?php echo number_format($totalCollection, 2) ?></span></th>
                                                        <?php
                                                            } else {
                                                        ?>
                                                            <th colspan="2">Total Purchase <span class="spanAmount"><?php echo number_format($inTotalPrice, 2); ?></span></th>
                                                            <th colspan="2">Total Due <span class="spanAmount"><?php echo number_format($totalCollection, 2) ?></span></th>
                                                        <?php
                                                            }
                                                        ?>                                                         
                                                    </tfoot>
