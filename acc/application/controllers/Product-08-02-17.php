...
class Product extends BC_Controller {
    ...
    public function updateStock() {
        ...
        if($this->input->post('adjustStock') != '') {
            if($this->input->post('productSign') != '') {
                $inputStock = "-".$this->input->post('adjustStock');
            } else {
                $inputStock = $this->input->post('adjustStock');
            }
        }
        $status = 0;
        if($this->input->post('adjustStock') != 0 && $this->input->post('adjustStock') < $this->input->post('dueProductOld')) {
            $status = 1;
        } else {
            $status = 2;
        }
        $updateData = array(
            'adjustProduct' => ($this->input->post('currentAdjustStock')+$inputStock),            
            'adjustStatus' => $status
        );
    ...
    public function mailSend() {
        ...
        $this->email->from($this->input->post("emailSender"), ucfirst($this->input->post("customName")));
