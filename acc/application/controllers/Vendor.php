...
class Vendor extends BC_Controller {
    private $timezone;
    private $systemDate;
    ...
    public function index() {
        ...
        if(!$this->is_logged_in()) {
            ...
            $data['totalVendor'] = $this->data_model->count_specific_table_without_where('list_business_member');
            $data['totalActivity'] = $this->data_model->count_specific_table_without_where('history_list_recent_activities');
            $data['activeVendor'] = $this->data_model->get_all_info_where_with_order('*', array('type' => 'vendor', 'status' => 1), 'id', 'desc', 'list_business_member');            
            $data['inActiveVendor'] = $this->data_model->get_all_info_where_with_order('*', array('type' => 'vendor', 'status' => 0), 'id', 'desc', 'list_business_member');            
            $data['countVendor'] = $this->data_model->count_specific_table_array('list_business_member', array('type' => 'vendor', 'status' => 1));
            $data['totalComment'] = $this->data_model->count_specific_table_without_where('list_all_comment'); 
            $data['main_content'] = 'vendor_list';
    ...
    public function add() {
        $this->benchmark->mark('generate_start');
        if(!$this->is_logged_in()) {
            $this->home('vendor/add/');
        } else if($this->is_logged_in() && $this->session->userdata('generate_token') == "") {
            $this->lock('vendor/add/');
        } else {
            $data['totalVendor'] = $this->data_model->count_specific_table_without_where('list_business_member');
            $data['totalActivity'] = $this->data_model->count_specific_table_without_where('history_list_recent_activities');
            $data['activeVendor'] = $this->data_model->get_all_info_where_with_order('*', array('type' => 'vendor', 'status' => 1), 'id', 'desc', 'list_business_member');            
    ...
    public function addVendor($parameter = '') {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $where = array("email" => $this->input->post('email'), "type" => 'vendor');
    ...
    public function getVendor() {
        $name = $this->data_model->get_all_info('*', array('id' => $this->input->post('id')), 'list_business_member');
        $arrayName = array('response' => $name[0]->name, 'nameOfContact' => $name[0]->nameOfContact, 'address1' => $name[0]->address1, 'address2' => $name[0]->address2, 'logo' => $name[0]->logo);
        echo json_encode($arrayName);
    }
